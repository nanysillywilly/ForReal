...
local Defaults = ReplicatedStorage:WaitForChild("Defaults")
...
function DropdownClass:SetOptionByString(optionName)
	for _, option in self.Button.ListFrame.ScrollFrame:GetChildren() do
		if not option:IsA("TextButton") then
			continue
		end

		if option.Label.Text == optionName then
			self:Set(option)
			break
		end
	end
...
function DropdownClass:GetText()
	return self._Selected.Label.Text
end
